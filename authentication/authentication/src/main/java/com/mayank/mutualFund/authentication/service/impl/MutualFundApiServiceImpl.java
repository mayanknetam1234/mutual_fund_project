...
import com.mayank.mutualFund.authentication.entity.csvEntity.MutualFundCsv;
...
public class MutualFundApiServiceImpl implements MutualFundApiService {
    ...
    @Override
    public void hitApiAndSaveIsin() throws Exception{

        String url = "https://mf.captnemo.in/kuvera/{isin}";
        List<MutualFundCsv> mutualFundCsvList=csvReaderService.readFunds("isin_list.csv");
       try (PrintWriter writer = new PrintWriter(new FileWriter("successful_isins.csv"))){
           int i=0;
           writer.println("ISIN");
    ...
    @Override
    public List<MutualFund> hitSuccessFullApi() {
        String url = "https://mf.captnemo.in/kuvera/{isin}";
        List<MutualFundCsv> mutualFundCsvList = csvReaderService.readFunds("successful_isins.csv");
